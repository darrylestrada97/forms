...
import { FormRandomService } from '../services/form-random.service';
@Component({
  selector: 'app-form-country-paged',
  templateUrl: './form-country-paged.component.html',
  styleUrls: ['./form-country-paged.component.css']
})
export class FormCountryPagedComponent implements OnInit {
  ...
  countrys: Country[]=[];
  countrysFiltered: Country[]=[];

  countrySelected: Country;

  constructor(private formrandomService: FormRandomService) { 
  }

  ngOnInit() {
    this.countrys = this.formrandomService.
    ...
    Object.assign(this.countrysFiltered, this.countrys);
  ...
  filter(){
    this.countrysFiltered = this.countrys.
      filter(fil => {
        let countryValid: boolean = false;
        let dateValid: boolean = false;
        let cepaVaild: boolean = false;



        
        if(this.countryFilter && this.countryFilter != ''){
          if(fil.$contry.toLowerCase().indexOf(this.countryFilter.toLowerCase()) != -1){
            countryValid = true;
          }
        }else{
          countryValid = true;
        ...
        if(this.dateFilter && this.dateFilter != ''){
          if(fil.$date.toLowerCase().indexOf(this.dateFilter.toLowerCase()) != -1){
            dateValid = true;
          }
        }else{
          dateValid = true;
        ...
        if(this.cepaFilter && this.cepaFilter != ''){
          if(fil.$cepa.toLowerCase().indexOf(this.cepaFilter.toLowerCase()) != -1){
            cepaVaild = true;
          }
        }else{
          cepaVaild = true;
        ...
        return  countryValid && dateValid && cepaVaild;
  ...
  onClick(rv: Country){
    this.countrySelected = rv;
  }

  removeRes(rv: Country){
    this.countrys.splice
      (this.countrys.indexOf(rv),1);
    this.countrysFiltered.splice
      (this.countrysFiltered.indexOf(rv),1);
  }
}
